...
- [ ] **Efficiency review:** Audit one run: which external HTTP calls are made multiple times for the same logical input? Add shared fetch or pass-through from an earlier node (e.g. “ACLED events” node → consumed by GEOINT and PROTEST).
