...
## Optional (neue Agents: CYBER, ENERGY, CIVIL_UNREST, DIPLO)
...
| **ACLED_EMAIL** | `geoint_agent.py`, `services/acled_auth.py` (CIVIL_UNREST-Agent entfernt; Stub in `civil unrest_stub.py`) | Deine myACLED-Registrierungs-E-Mail. **OAuth (empfohlen):** zusammen mit `ACLED_PASSWORD` für Token-Auth. |
...
| **CIVIL_UNREST_USE_ACLED_API** | *entfällt* (CIVIL_UNREST-Modul entfernt) | Früher: Read-API im Protest-Agent. Nicht mehr wirksam bis eine neue Implementierung die Variable wieder auswertet. |
| **CIVIL_UNREST_GDELT_EVENT_ROOTS** | `backend/services/gdelt_bigquery.py` | Komma-separierte CAMEO-`EventRootCode`-Werte für `fetch_gdelt_civil unrest_events_summary` (Default: `12,13,14,15,17,18`). |
| **CIVIL_UNREST_GDELT_LOOKBACK_DAYS** | `backend/services/gdelt_bigquery.py` | Lookback für CIVIL_UNREST-Events-BQ (sonst `GDELT_BQ_LOOKBACK_DAYS`). |
| **CIVIL_UNREST_GKG_BQ_ENABLED** | `backend/services/gdelt_bigquery.py` | Default `1`. Setze `0`, um GKG-BigQuery für CIVIL_UNREST zu überspringen. |
| **CIVIL_UNREST_GKG_LOOKBACK_DAYS** | `backend/services/gdelt_bigquery.py` | GKG-Fenster in Tagen (Default `3`, max. 14). |
| **CIVIL_UNREST_GKG_BQ_TIMEOUT_SEC** | `backend/services/gdelt_bigquery.py` | Query-Timeout GKG (Default `22`). |
...
| **CIVIL_UNREST_RUN_ASYNC_TIMEOUT_S** | *entfällt* | Früher: Timeout für den entfernten Protest-Agent. |
...
**Ohne ACLED-API:** Auf der ACLED-Webseite gibt es öffentlich einsehbare Crisis-Live-Daten, z. B. **[Iran Crisis Live](https://acleddata.com/iran-crisis-live)** – tägliche Updates, Karten und Analysen zum Iran-Konflikt (täglich 10:30 EST / 15:30 CET). Der **CIVIL_UNREST**-Slot ist nur noch ein **Stub** (kein ACLED-Fetch). **GEOINT** und **Aggregated-CSV** (`acled_aggregated`) nutzen weiterhin `ACLED_EMAIL`/`ACLED_PASSWORD`, wo implementiert.
...
3. **Keine Credentials:** Wenn weder OAuth noch Legacy-Key gesetzt sind, liefern GEOINT/Heatmap/aggregierte Pfade keine ACLED-Daten; CIVIL_UNREST liefert ohnehin nur Stub-Daten. Die Meldung „ACLED OAuth: no credentials“ kann dann in den Logs erscheinen.
...
| **AISSTREAM_API_KEY** | `backend/agents/airstream_client.py` / Chokepoint – AISStream (aisstream.io), WebSocket mit BoundingBox (Hormuz, Bab el-Mandeb, Suez). **Legacy-Aliase:** `AIRSTREAM_API_KEY`, `AIRSTREAM_API`. | [aisstream.io/apikeys](https://aisstream.io/apikeys). Doku: [aisstream.io/documentation](https://aisstream.io/documentation). Optional: **AISSTREAM_COLLECT_SECONDS** (Default 15), **AISSTREAM_FILTER_MESSAGE_TYPES** (kommagetrennt), **AISSTREAM_FILTERS_SHIP_MMSI** (offizielles MMSI-Filter; kommagetrennt). |
...
**Hormuz Tankers (SIGINT):** Die Liste „Hormuz Tankers“ wird aus dem **Chokepoint-Agent** (AISStream) übernommen, wenn **AISSTREAM_API_KEY** (oder ein Legacy-Alias) gesetzt ist. Ohne Key bleibt die Liste leer. **VesselFinder ist aus dem Code entfernt** (keine Naval-Vessels- noch Tanker-Quelle mehr).
...
| **NEWSLETTER_SEND_TIMEZONE** | `backend/main.py` | IANA timezone for the daily send (default: `Europe/Berlin` — covers CET/CEST). Used with `NEWSLETTER_SEND_HOUR` / `NEWSLETTER_SEND_MINUTE` when `NEWSLETTER_SEND_UTC_HOUR` is **not** set. |
| **NEWSLETTER_SEND_HOUR** | `backend/main.py` | Local hour `0–23` for the in-process daily job (default: `10` → 10:00 morning in `NEWSLETTER_SEND_TIMEZONE`). |
| **NEWSLETTER_SEND_MINUTE** | `backend/main.py` | Local minute `0–59` (default: `0`). |
| **NEWSLETTER_SEND_UTC_HOUR** | `backend/main.py` | **Legacy:** if this env var is present, it overrides the timezone settings: fixed UTC hour `0–23` for the daily job (previous default was `6`). Prefer `NEWSLETTER_SEND_TIMEZONE` + hour/minute for CET/CEST. External cron may call `POST /api/newsletter/send-daily` with `X-Newsletter-Secret` instead of the in-process scheduler. |
...
| **GDELT** (Protest) | — | CIVIL_UNREST-Agent entfernt; BQ-Helfer in `gdelt_bigquery.py` optional | — |
