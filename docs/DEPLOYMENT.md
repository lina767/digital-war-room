...
- [ ] **Umgebungsvariablen in Railway setzen** (unter Variables):
  ...
  - **Optional:**  
    `POLYMARKET_BUILDER_API_KEY`, `ACLED_EMAIL` + `ACLED_PASSWORD` (ACLED OAuth für CIVIL_UNREST/GEOINT/Heatmap; siehe [ACLED Getting started](https://acleddata.com/api-documentation/getting-started) und [ACLED-TROUBLESHOOTING.md](ACLED-TROUBLESHOOTING.md)), `SHODAN_API_KEY`, `CLOUDFLARE_RADAR_API_TOKEN`, `LIVEUAMAP_API_KEY` (GEOINT: Liveuamap Lebanon/Iran, kostenpflichtige API), `OTX_API_KEY` (CYBER: AlienVault OTX), `AGSI_API_KEY` (ENERGY: EU-Gasspeicher AGSI+).
...
- **CORS:** Bei eigener Domain in Railway `CORS_ORIGINS` setzen (Abschnitt 4). Siehe **[docs/SECURITY.md](SECURITY.md)** für Rate Limiting, Input-Sanitization und API-Key-Rotation (ohne Supabase).
