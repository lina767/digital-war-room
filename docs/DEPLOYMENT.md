...
- [ ] **Umgebungsvariablen in Railway setzen** (unter Variables):
  ...
  - **Optional:**  
    `POLYMARKET_BUILDER_API_KEY`, `ACLED_EMAIL` + `ACLED_PASSWORD` (ACLED OAuth für PROTEST/GEOINT/Heatmap; siehe [ACLED Getting started](https://acleddata.com/api-documentation/getting-started)), `SHODAN_API_KEY`, `CLOUDFLARE_RADAR_API_TOKEN`, `LIVEUAMAP_API_KEY` (GEOINT: Liveuamap Lebanon/Iran, kostenpflichtige API), `UCDP_API_TOKEN` (GEOINT: Uppsala Conflict Data Program),  `OTX_API_KEY` (CYBER: AlienVault OTX), `AGSI_API_KEY` (ENERGY: EU-Gasspeicher AGSI+).
  - **Observability (Tracing):** Standardmäßig ist **LangSmith** deaktiviert (`LANGCHAIN_TRACING_V2=false`). Stattdessen kann **OpenTelemetry (OTEL)** genutzt werden: Setze `OTEL_EXPORTER_OTLP_ENDPOINT` (z. B. `http://localhost:4317` für Jaeger gRPC). Optional: `OTEL_SERVICE_NAME=digital-war-room`. Traces (LLM-, Tool-, Chain-Spans) gehen dann an den konfigurierten OTLP-Endpoint (z. B. **Jaeger**). Jaeger lokal starten: `docker run -d --name jaeger -p 16686:16686 -p 4317:4317 -p 4318:4318 jaegertracing/all-in-one:latest` – UI unter `http://localhost:16686`. Bei Remote-Jaeger Host/Port im Endpoint anpassen. LangSmith weiter nutzbar mit `LANGCHAIN_TRACING_V2=true` und `LANGCHAIN_API_KEY` (dann ggf. doppelte Traces).
