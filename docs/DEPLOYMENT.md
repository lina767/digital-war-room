...
- [ ] **Umgebungsvariablen in Railway setzen** (unter Variables):
  ...
  - **Newsletter (Daily Briefing by email):**  
    `RESEND_API_KEY`, `NEWSLETTER_FROM`, `FRONTEND_URL` (for confirm/unsubscribe links). Daily job runs at 06:00 UTC (or `NEWSLETTER_SEND_UTC_HOUR`). Optional: `NEWSLETTER_CRON_SECRET` if calling `POST /api/newsletter/send-daily` from external cron. See [docs/NEWSLETTER-SPEC.md](NEWSLETTER-SPEC.md) and [docs/API-KEYS.md](API-KEYS.md#newsletter-daily-briefing-by-email).
