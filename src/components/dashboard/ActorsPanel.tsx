...
export function ActorsPanel({ conflictData, activeConflict }: ActorsPanelProps) {
  ...
  return (
    <IntelPanel
      ...
      headerRight={<span className="text-[11px] text-muted-foreground">{actors.length}</span>}
      ...
      <ul className="divide-y divide-border/60 max-h-[320px] overflow-y-auto">
        {actors.map((actor) => (
          <li key={actor.id}>
            ...
            >
              <CollapsibleTrigger className="w-full px-3 py-2 flex items-center gap-2 text-left hover:bg-muted/30 transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-primary/50 rounded">
                <span className="font-mono text-xs font-medium truncate flex-1 min-w-0">
                  {actor.name}
                </span>
                <span
                  className={`shrink-0 text-[11px] font-mono px-1.5 py-0.5 rounded border ${ROLE_STYLES[actor.role]}`}
                >
