...
/**
 * Searchable source directory embedded under Documentation → Source Directory
 * (replaces the standalone /sources page).
 */
export function SourceDirectoryDoc() {
  ...
  return (
    <section className="rounded-xl border border-border bg-card/25 p-5 sm:p-8 mt-6" aria-label="Searchable source directory">
      <div className="flex items-center gap-2 mb-6">
        <Database className="h-5 w-5 text-muted-foreground shrink-0" aria-hidden />
        <h2 className="text-lg sm:text-xl font-semibold tracking-tight">Live source directory</h2>
      </div>
      <p className="text-sm text-muted-foreground mb-6 max-w-3xl">
        Search and filter all data sources used by the platform (same data as the former standalone Source Directory page).
        Derived dashboard outputs (e.g. Global Impact, Sanctions Compliance) are not listed as separate sources.
      </p>
      ...
      <div className="mb-6 p-3 rounded-lg border border-border bg-muted/30 text-xs text-muted-foreground">
        ...
        <ul className="space-y-0.5">
          <li>
            <strong>Official</strong> – Government, UN, or official institutions (e.g. NASA, OFAC, CISA, EU, UN/ICJ).
          </li>
          <li>
            <strong>Curated</strong> – Verified APIs with clear terms (e.g. ACLED, NewsAPI, Alpha Vantage, GDELT).
          </li>
          <li>
            <strong>Community</strong> – Open feeds, OSINT, community-sourced (e.g. ADSB, RSS, Telegram, OONI).
          </li>
          <li>
            <strong>Supplementary</strong> – Optional or auxiliary data (e.g. custom GeoJSON, third-party aggregators).
          </li>
        </ul>
      </div>
      ...
      {filtered.length === 0 && (
        <p className="text-sm text-muted-foreground py-8 text-center">No sources match your search. Try a different query or clear filters.</p>
      )}
    </section>
  );
}
  ...
  return (
    <li className="rounded-lg border border-border bg-card/50 p-4 sm:p-5 hover:border-primary/30 transition-colors">
      <div className="flex flex-wrap items-start justify-between gap-2 mb-2">
        <h3 className="text-base sm:text-lg font-semibold tracking-tight">{source.name}</h3>
