...
import { extractToc, slugify } from "@/lib/documentationToc";
...
export function DocsArticle({ markdown, className }: DocsArticleProps) {
  ...
  return (
    <article className={cn("rounded-xl border border-border bg-card/25 p-5 sm:p-8", className)}>
...
export function DocsToc({ markdown }: { markdown: string }) {
  ...
  return (
    <nav className="rounded-xl border border-border bg-card/25 p-4" aria-labelledby="docs-toc-heading">
      <h2 id="docs-toc-heading" className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground mb-3">
        On this page
      </h2>
      {toc.length === 0 ? (
