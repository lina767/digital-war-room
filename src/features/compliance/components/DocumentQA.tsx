...
import type { DocumentQAResponse } from "../types/compliance.types";

export function DocumentQA({ data }: { data: ConflictData | null }) {
  const { buildDocumentQAContext, conflictKey } = useComplianceData(data);
    ...
    try {
      ...
      const resp = await fetch(`${base}/api/compliance/document-qa`, {
        ...
        body: JSON.stringify({
          ...
          context: buildDocumentQAContext(),
        }),
...
export { DocumentQA as DocumentQASection };
